...
public class SendEmailServlet extends HttpServlet{
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		// 이메일 제목, 보내는 사람, 보내는 사람 이메일, 이메일 받는 사람
		
		
		
		String emailTitle = "[community] 에서 보내는 인증 메일";
		String fromUser =  "관리자";
		String fromUserEmail = "";
		String toUser = inputEmail;
		
		final String smtpEmail = ""; // 이메일
		final String password = ""; // 발급받은 비밀번호
		...
		try {
			...
			message.setFrom(new InternetAddress(fromUserEmail,fromUser)); // 송신자(보내는 사람) 지정
			message.addRecipient(RecipientType.TO, new InternetAddress(toUser)); // 수신자 (받는 사람) 지정
			message.setSubject(emailTitle); // 이메일 제목 지정
